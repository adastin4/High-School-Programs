...
int main(void){
    ...
    puts("");
    size_t *path = shortestPath(graph, N, 0, 3);
    for(size_t i = 0; path[i] != (size_t)-1 && i < N;++i){
        printf("%zu ", path[i]);
    }
    puts("");
...
size_t *dijkstra(unsigned int **graph, size_t n, size_t source){
    ...
    while(!isEmpty(unvisited, n) && iter++ < n){
        ...
        for(i = 0; i<n; ++i){
            ...
            if(i != current && graph[current][i] < (unsigned int)-1 
...
size_t *shortestPath(unsigned int **graph, size_t n, size_t source, size_t dest){ 
    ...
    size_t curr = dest, prev = previous[dest], index;
